...
- Не использовать **владеющие** сырые указатели: владение выражать через `std::unique_ptr`, `std::shared_ptr` или RAII-объекты; невладеющие сырые указатели (`T*`) допустимы когда ссылка не подходит по семантике (nullable, переназначаемые члены класса)
- Перед каждым `// NOLINT(...)` писать короткое объяснение, почему подавление допустимо
- Не вызывать `std::getenv` вне `main.cpp`
