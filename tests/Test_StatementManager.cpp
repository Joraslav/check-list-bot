...
class StatementManagerTest : public ::testing::Test {
 protected:
	...
	int64_t InsertTaskRaw(int64_t user_id, std::string_view text, TaskStatus status) {
		Statement stmt(
			db, "INSERT INTO tasks (user_id, text, status) VALUES (?, ?, ?) RETURNING id");
		stmt.bind(1, user_id);
	...
	[[nodiscard]] int CountRows(std::string_view table_name) const {
		Statement stmt(db, "SELECT COUNT(*) FROM "s + std::string(table_name));
		const bool has_row = stmt.executeStep();
		EXPECT_TRUE(has_row);
...
TEST_F(StatementManagerTest, Get_Throws_For_Invalid_Statement_Type) {
	const auto invalid_type = static_cast<StatementType>(static_cast<int>(StatementType::COUNT) + 1);
...
TEST_F(StatementManagerTest, InsertTask_And_SelectAllTasks_Work_And_Keep_Order_By_Id) {
	...
	EXPECT_EQ(select_all_stmt.getColumn(1).getString(), "task-1"s);
	...
	EXPECT_EQ(select_all_stmt.getColumn(1).getString(), "task-2"s);
...
TEST_F(StatementManagerTest, SelectTasksByStatus_Filters_Rows_Correctly) {
	InsertUserRaw(1001, "alice"sv);
	const auto active_id = InsertTaskRaw(1001, "active", TaskStatus::ACTIVE);
	const auto completed_id = InsertTaskRaw(1001, "completed", TaskStatus::COMPLETED);
	...
	EXPECT_EQ(select_by_status_stmt.getColumn(1).getString(), "completed"s);
...
TEST_F(StatementManagerTest, UpdateStatus_Updates_Only_Matching_Task) {
	...
	const int64_t alice_task = InsertTaskRaw(1001, "alice-task", TaskStatus::ACTIVE);
	const int64_t bob_task = InsertTaskRaw(2002, "bob-task", TaskStatus::ACTIVE);
...
TEST_F(StatementManagerTest, EditTask_And_DeleteTask_Modify_Data_As_Expected) {
	InsertUserRaw(1001, "alice"sv);
	const int64_t keep_task = InsertTaskRaw(1001, "old-text", TaskStatus::ACTIVE);
	const int64_t delete_task = InsertTaskRaw(1001, "to-delete", TaskStatus::ACTIVE);
	...
	EXPECT_EQ(TaskTextById(keep_task), "new-text"s);
...
TEST_F(StatementManagerTest, DeleteAllUserTasks_Deletes_Only_Target_User_Rows) {
	InsertUserRaw(1001, "alice"sv);
	InsertUserRaw(2002, "bob"sv);
	InsertTaskRaw(1001, "a-1", TaskStatus::ACTIVE);
	InsertTaskRaw(1001, "a-2", TaskStatus::COMPLETED);
	InsertTaskRaw(2002, "b-1", TaskStatus::ACTIVE);
...
TEST_F(StatementManagerTest, VerifyOwnership_And_UserStatistics_Return_Correct_Data) {
	InsertUserRaw(1001, "alice"sv);
	InsertUserRaw(2002, "bob"sv);
	const int64_t alice_active = InsertTaskRaw(1001, "a-active", TaskStatus::ACTIVE);
	static_cast<void>(InsertTaskRaw(1001, "a-completed", TaskStatus::COMPLETED));
	const int64_t bob_active = InsertTaskRaw(2002, "b-active", TaskStatus::ACTIVE);
